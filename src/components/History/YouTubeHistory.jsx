...
const YouTubeHistory = () => {
  ...
  const [historyVideosLoading, setHistoryVideosLoading] = useState(false);

  const fetchHistory = async () => {
    setHistoryVideosLoading(true);
    try {
      const response = await fetch(`${HOST}/api/v1/users/getWatchHistory`, {
        method: "GET",
        headers: {
      ...
      if (data.statusCode === 200) {
        const historyList = data.data.map((item) => {
          return {
            ...
            duration:
              Math.round(Math.floor(item.duration) / 60) +
              ":" +
              (Math.floor(item.duration) % 60 < 10 ? "0" : "") +
              (Math.floor(item.duration) % 60),
            avatar: item.owner.avatar,
            watchedAt: timeAgo(item.updatedAt),
          };
        });
        setHistoryVideos(historyList);
      } else {
        setHistoryVideos([]);
      }
    } catch (error) {
